...
def span_f1(predicts, span_label_ltoken, real_span_mask_ltoken):
    ...
    '''
    pred_label_idx = mindspore.max(predicts, dim=-1)[1]     # (bs, n_span)
    ...
    correct_pred = mindspore.sum(all_correct)
    total_pred = mindspore.sum(pred_label_idx != 0)
    total_golden = mindspore.sum(span_label_ltoken != 0)

    return mindspore.stack([correct_pred, total_pred, total_golden])
...
def span_f1_prune(all_span_idxs, predicts, span_label_ltoken, real_span_mask_ltoken):
    ...
    return mindspore.stack([correct_pred, total_pred, total_golden]),pred_label_idx
...
def get_pruning_predIdxs(pred_label_idx, all_span_idxs, span_probs):
    ...
    pred_label_idx_new = mindspore.LongTensor(pred_label_idx_new)
