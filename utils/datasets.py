...
def collate_fn(batch):
    ...
    for idx in range(len(['ori_feas', 'cont_feas'])):
        features.append({
            'input_ids': mindspore.LongTensor([sample[idx][0] for sample in batch]),
            'input_mask': mindspore.LongTensor([sample[idx][1] for sample in batch]),
            'segment_ids': mindspore.LongTensor([sample[idx][2] for sample in batch]),
            'span_token_idxes': mindspore.LongTensor([sample[idx][3] for sample in batch]),
            'span_labels': mindspore.LongTensor([sample[idx][4] for sample in batch]),
            'span_weights': mindspore.Tensor([sample[idx][5] for sample in batch]),
            'morph_idxes': mindspore.LongTensor([sample[idx][6] for sample in batch]),
            'span_lens': mindspore.LongTensor([sample[idx][7] for sample in batch]),
            'span_masks': mindspore.LongTensor([sample[idx][8] for sample in batch]),
            'cont_span_idx': mindspore.LongTensor([sample[idx][9] for sample in batch]),
            'span_word_idxes': mindspore.LongTensor([sample[idx][10] for sample in batch])
        })
