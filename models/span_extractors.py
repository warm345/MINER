...
from mindspore.nn.functional import embedding
...
class TokenEmbedder(mindspore.nn.Module):
    """
...
class TimeDistributed(mindspore.nn.Module):
    """
    ...
    def forward(self, *inputs, pass_through: List[str] = None, **kwargs):
        ...
        for key, value in kwargs.items():
            if isinstance(value, mindspore.Tensor) and key not in pass_through:
...
class Embedding(TokenEmbedder):
    """
    A more featureful embedding module than the default in mindspore.  Adds the ability to:
    ...
    weight : `mindspore.FloatTensor`, optional (default=`None`)
    ...
    sparse : `bool`, optional (default=`False`)
        Whether or not the mindspore backend should use a sparse representation of the embedding weight.
    ...
    pretrained_file : `str`, optional (default=`None`)
        ...
        path to a local file or a URL of a (cached) remote file. Two formats are supported:
            * hdf5 file - containing an embedding matrix in the form of a mindspore.Tensor;
    ...
    def __init__(
        ...
        weight: mindspore.FloatTensor = None,
        ...
        if weight is not None:
            ...
            self.weight = mindspore.nn.Parameter(weight, requires_grad=trainable)

        else:
            weight = mindspore.FloatTensor(num_embeddings, embedding_dim)
            self.weight = mindspore.nn.Parameter(weight, requires_grad=trainable)
        ...
        if projection_dim:
            self._projection = mindspore.nn.Linear(embedding_dim, projection_dim)
        else:
    ...
    def forward(self, tokens: mindspore.Tensor) -> mindspore.Tensor:
...
class SpanExtractor(mindspore.nn.Module):
    """
    ...
    def forward(
        self,
        sequence_tensor: mindspore.FloatTensor,
        span_indices: mindspore.LongTensor,
        sequence_mask: mindspore.BoolTensor = None,
        span_indices_mask: mindspore.BoolTensor = None,
    ):
        """
        ...
        # Parameters
        sequence_tensor : `mindspore.FloatTensor`, required.
            A tensor of shape (batch_size, sequence_length, embedding_size)
            representing an embedded sequence of words.
        span_indices : `mindspore.LongTensor`, required.
            A tensor of shape `(batch_size, num_spans, 2)`, where the last
            dimension represents the inclusive start and end indices of the
            span to be extracted from the `sequence_tensor`.
        sequence_mask : `mindspore.BoolTensor`, optional (default = `None`).
            A tensor of shape (batch_size, sequence_length) representing padded
            elements of the sequence.
        span_indices_mask : `mindspore.BoolTensor`, optional (default = `None`).
...
class SpanExtractorWithSpanWidthEmbedding(SpanExtractor):
    ...
    # Returns
    span_embeddings : `mindspore.FloatTensor`.
        A tensor of shape `(batch_size, num_spans, embedded_span_size)`,
        where `embedded_span_size` depends on the way spans are represented.
    """
    ...
    def forward(
        ...
        if self._span_width_embedding is not None:
            ...
            span_embeddings = mindspore.cat([span_embeddings, span_width_embeddings], -1)
...
class EndpointSpanExtractor(SpanExtractorWithSpanWidthEmbedding):
    ...
    def __init__(
        ...
        if use_exclusive_start_indices:
            self._start_sentinel = Parameter(mindspore.randn([1, 1, int(input_dim)]))
    ...
    def _embed_spans(
        ...
        if not self._use_exclusive_start_indices:
            ...
            # We'll check the indices here at runtime, because it's difficult to conll_debug
...
def combine_initial_dims(tensor: mindspore.Tensor) -> mindspore.Tensor:
    """
...
def bucket_values(
    distances: mindspore.Tensor, num_identity_buckets: int = 4, num_total_buckets: int = 10
) -> mindspore.Tensor:
    """
    ...
    # Parameters
    distances : `mindspore.Tensor`, required.
...
def batched_index_select(
    ...
    indices: mindspore.LongTensor,
    flattened_indices: Optional[mindspore.LongTensor] = None,
) -> mindspore.Tensor:
    """
    ...
    mentions.
    The key reason this can't be done with basic mindspore functions is that we want to be able to use look-up
    tensors with an arbitrary number of dimensions (for example, in the coref model, we don't know
    a-priori how many spans we are looking up).
    # Parameters
    target : `mindspore.Tensor`, required.
        A 3 dimensional tensor of shape (batch_size, sequence_length, embedding_size).
        This is the tensor to be indexed.
    indices : `mindspore.LongTensor`
        A tensor of shape (batch_size, ...), where each element is an index into the
        `sequence_length` dimension of the `target` tensor.
    flattened_indices : `Optional[mindspore.Tensor]`, optional (default = `None`)
        An optional tensor representing the result of calling `flatten_and_batch_shift_indices`
        on `indices`. This is helpful in the case that the indices can be flattened once and
        cached for many batch lookups.
    # Returns
    selected_targets : `mindspore.Tensor`
...
def flatten_and_batch_shift_indices(indices: mindspore.Tensor, sequence_length: int) -> mindspore.Tensor:
    """
    ...
    ```python
        indices = mindspore.ones([2,3], dtype=mindspore.long)
    ...
    ```
    # Parameters
    indices : `mindspore.LongTensor`, required.
    sequence_length : `int`, required.
        The length of the sequence the indices index into.
        This must be the second dimension of the tensor.
    # Returns
    offset_indices : `mindspore.LongTensor`
    """
    # Shape: (batch_size)
    if mindspore.max(indices) >= sequence_length or mindspore.min(indices) < 0:
        raise Exception(
...
def get_range_vector(size: int, device: int) -> mindspore.Tensor:
    """
    Returns a range vector with the desired size, starting at 0. The CUDA implementation
    is meant to avoid copy data from CPU to GPU.
    """
    if device > -1:
        return mindspore.cuda.LongTensor(size, device=device).fill_(1).cumsum(0) - 1
    else:
        return mindspore.arange(0, size, dtype=mindspore.long)


def get_device_of(tensor: mindspore.Tensor) -> int:
    """
...
def combine_tensors(combination: str, tensors: List[mindspore.Tensor]) -> mindspore.Tensor:
    """
    ...
    just use something like `mindspore.cat([x_tensor, y_tensor, x_tensor * y_tensor])`.  This
    ...
    return mindspore.cat(to_concatenate, dim=-1)
...
def uncombine_initial_dims(tensor: mindspore.Tensor, original_size: mindspore.Size) -> mindspore.Tensor:
    """
