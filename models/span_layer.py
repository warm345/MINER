...
class SpanLayer(mindspore.nn.Module):
    ...
    def forward(
        ...
        span_morph_rep = mindspore.sum(span_morph_rep, dim=2)  # (bs, n_span, dim)

        spanlen_rep = self.spanLen_embedding(span_lens)  # (bs, n_span, len_dim)
        spanlen_rep = F.relu(spanlen_rep)

        all_span_rep = mindspore.cat((all_span_rep, spanlen_rep, span_morph_rep), dim=-1)
