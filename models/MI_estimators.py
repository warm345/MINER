...
class UpperWithPosterior(UpperBound):
    ...
    def loglikeli(self, y_samples, mu, log_var):
        ...
        return (-0.5 * (mu - y_samples) ** 2 / log_var.exp()
                + log_var
                + mindspore.log(math.pi)
                ).sum(dim=1).mean(dim=0)

    # 从正态分布中 sample 样本
    def get_sample_from_param_batch(self, mean, log_var, sample_size):
        bsz, seqlen, tag_dim = mean.shape
        z = mindspore.randn(bsz, sample_size, seqlen, tag_dim).to(self.device)

        z = z * mindspore.exp(0.5 * log_var).unsqueeze(1).expand(-1, sample_size, -1, -1) + \
...
class vCLUB(UpperBound):
    ...
    def mi_est(self, y_samples, y_n_samples):
        ...
        """
        positive = mindspore.zeros_like(y_samples)
    ...
    def consine(self, y_samples, y_n_samples):
        ...
        :return:
               mi estimation [nsample, 1]
        """
        return mindspore.cosine_similarity(y_samples, y_n_samples, dim=-1)
...
class InfoNCE(nn.Module):
    ...
    def forward(self, x_samples, y_samples):  # samples have shape [sample_size, dim]
        ...
        T0 = self.F_func(mindspore.cat([x_samples, y_samples], dim=-1))
        try:
            T1 = self.F_func(mindspore.cat([x_tile, y_tile], dim=-1))  # [s_size, s_size, 1]
        except Exception:
            print('Debug')

        lower_bound = T0 - T1.logsumexp(dim=1)  # mindspore.log(T1.exp().mean(dim = 1)).mean()
...
def kl_div(param1, param2):
    ...
    KL = 0.5 * (
            mindspore.sum(mindspore.log(cov2), dim=1)
            - mindspore.sum(mindspore.log(cov1), dim=1)
            - var_len
            + mindspore.sum(cov2_inv * cov1, dim=1)
            + mindspore.bmm(temp, mean_diff.view(bsz, -1, 1)).view(bsz)
    )
...
def kl_norm(mu, log_var):
    ...
    return -0.5 * mindspore.sum(1 + log_var - mu ** 2 - log_var.exp(), dim=1).mean()
