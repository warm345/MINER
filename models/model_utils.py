...
def valid_sequence_output(sequence_output, valid_mask, attention_mask):
    ...
    valid_output = mindspore.zeros(batch_size, max_len, feat_dim, dtype=mindspore.float32,
                               device='cuda' if mindspore.cuda.is_available() else 'cpu')
    valid_attention_mask = mindspore.zeros(batch_size, max_len, dtype=mindspore.long,
                                       device='cuda' if mindspore.cuda.is_available() else 'cpu')
...
def get_random_span(x_span, x_weights, y_span, y_weights,
    ...
    x_span_idxes = mindspore.zeros(
        bsz, dtype=mindspore.int64,
        device='cuda' if mindspore.cuda.is_available() else 'cpu'
    )
    y_span_idxes = mindspore.zeros(
        bsz, dtype=mindspore.int64,
        device='cuda' if mindspore.cuda.is_available() else 'cpu'
    )
