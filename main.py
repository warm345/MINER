...
def get_args():
    ...
    mindspore.cuda.set_device(args.gpu_id)
    device = mindspore.device("cuda", args.gpu_id)
    args.device = device
...
def train(args, model, tokenizer, labels):
    ...
    for _ in train_iterator:
        epoch_num += 1
        train_dataloader = ds.GeneratorDataset(train_examples)

        logger.info("Training epoch num {0}".format(epoch_num))
        epoch_iterator = tqdm(train_dataloader, desc="Iteration")

        for step, batch in epoch_iterator.create_dict_iterator():
            model.train()
            ...
            # clip gradients
            mindspore.ops.clip_by_global_norm(model.parameters(), args.max_grad_norm)
            optimizer.step()
...
def evaluate(args, model, tokenizer, labels, mode='test', prefix='', examples=None):
    ...
    all_counts = mindspore.stack([x[f'span_f1s'] for x in dev_outputs]).sum(0)
